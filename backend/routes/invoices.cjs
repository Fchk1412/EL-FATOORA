...
// 🔹 Helper: Generate XML content using ONLY string concatenation
function generateInvoiceXML(invoiceData) {
  const { invoice, items } = invoiceData;

  // Helper function to sanitize values for XML
  ...
  // Create XML manually using ONLY string concatenation - NO xmlbuilder2
  let xml = '<?xml version="1.0" encoding="UTF-8"?>\n';
  xml += '<TEIF controllingAgency="TTN" version="1.8.8">\n';
  
  // Invoice Header
  xml += '  <InvoiceHeader>\n';
  xml += `    <MessageSenderIdentifier type="I-01">${sanitizeXMLValue(invoice.company_client_code) || '0000000000000'}</MessageSenderIdentifier>\n`;
  xml += `    <MessageRecieverIdentifier type="I-01">${sanitizeXMLValue(invoice.client_tax_number) || '0000000000000'}</MessageRecieverIdentifier>\n`;
  xml += '  </InvoiceHeader>\n';
  ...
  // Document info
  xml += '    <Bgm>\n';
  xml += `      <DocumentIdentifier>${sanitizeXMLValue(invoice.invoice_number)}</DocumentIdentifier>\n`;
  xml += '      <DocumentType code="I-11">Facture</DocumentType>\n';
  xml += '    </Bgm>\n';
  
  // Dates
  xml += '    <Dtm>\n';
  xml += `      <DateText format="ddMMyy" functionCode="I-31">${formatDateForXML(invoice.invoice_date)}</DateText>\n`;
  if (invoice.service_period_start && invoice.service_period_end) {
    xml += `      <DateText format="ddMMyy-ddMMyy" functionCode="I-36">${formatDateForXML(invoice.service_period_start)}-${formatDateForXML(invoice.service_period_end)}</DateText>\n`;
  }
  xml += `      <DateText format="ddMMyy" functionCode="I-32">${formatDateForXML(invoice.due_date)}</DateText>\n`;
  xml += '    </Dtm>\n';
  ...
  xml += '        <Nad>\n';
  xml += `          <PartnerIdentifier type="I-01">${sanitizeXMLValue(invoice.company_client_code) || '0000000000000'}</PartnerIdentifier>\n`;
  xml += `          <PartnerName nameType="Qualification">${sanitizeXMLValue(invoice.company_name)}</PartnerName>\n`;
  ...
  xml += '        <Nad>\n';
  xml += `          <PartnerIdentifier type="I-01">${sanitizeXMLValue(invoice.client_tax_number) || '0000000000000'}</PartnerIdentifier>\n`;
  xml += `          <PartnerName nameType="Qualification">${sanitizeXMLValue(invoice.client_name)}</PartnerName>\n`;
  xml += '          <PartnerAdresses lang="fr">\n';
  xml += `            <AdressDescription>${sanitizeXMLValue(invoice.client_address) || ''}</AdressDescription>\n`;
  xml += '            <Street></Street>\n';
  xml += `            <CityName>${sanitizeXMLValue(invoice.client_city) || ''}</CityName>\n`;
  xml += `            <PostalCode>${sanitizeXMLValue(invoice.client_postal_code) || ''}</PostalCode>\n`;
  xml += '            <Country codeList="ISO_3166-1">TN</Country>\n';
  xml += '          </PartnerAdresses>\n';
  xml += '        </Nad>\n';
  xml += '      </PartnerDetails>\n';
  ...
  if (invoice.payment_terms || invoice.payment_rib) {
    ...
    xml += `          <PaymentTearmsDescription>${sanitizeXMLValue(invoice.payment_terms) || ''}</PaymentTearmsDescription>\n`;
    xml += '        </Pyt>\n';
    
    if (invoice.payment_rib) {
      xml += '        <Pyt>\n';
      xml += '          <PaymentTearmsTypeCode>I-115</PaymentTearmsTypeCode>\n';
      xml += `          <PaymentTearmsDescription>RIB: ${sanitizeXMLValue(invoice.payment_rib)}</PaymentTearmsDescription>\n`;
      xml += '        </Pyt>\n';
    }
  ...
  items.forEach((item, index) => {
    ...
    xml += '        <LinImd lang="fr">\n';
    xml += `          <ItemCode>${sanitizeXMLValue(item.product_code) || 'PROD'}</ItemCode>\n`;
    xml += `          <ItemDescription>${sanitizeXMLValue(item.product_name)}</ItemDescription>\n`;
    xml += '        </LinImd>\n';
    xml += '        <LinQty>\n';
    ...
    xml += '        <LinMoa>\n';
    xml += '          <MoaDetails>\n';
    xml += '            <Moa amountTypeCode="I-183" currencyCodeList="ISO_4217">\n';
    xml += `              <Amount currencyIdentifier="TND">${Number(item.unit_price).toFixed(3)}</Amount>\n`;
    xml += '            </Moa>\n';
    xml += '          </MoaDetails>\n';
    xml += '          <MoaDetails>\n';
    xml += '            <Moa amountTypeCode="I-171" currencyCodeList="ISO_4217">\n';
    xml += `              <Amount currencyIdentifier="TND">${Number(item.amount_excl_tax).toFixed(3)}</Amount>\n`;
    xml += '            </Moa>\n';
    xml += '          </MoaDetails>\n';
    xml += '        </LinMoa>\n';
    xml += '      </Lin>\n';
  ...
  xml += `          <Amount currencyIdentifier="TND">${Number(invoice.total_excl_tax).toFixed(3)}</Amount>\n`;
  xml += '        </Moa>\n';
  xml += '      </AmountDetails>\n';
  xml += '      <AmountDetails>\n';
  xml += '        <Moa amountTypeCode="I-180" currencyCodeList="ISO_4217">\n';
  xml += `          <Amount currencyIdentifier="TND">${Number(invoice.total_amount).toFixed(3)}</Amount>\n`;
  xml += `          <AmountDescription lang="fr">${sanitizeXMLValue(invoice.total_amount_words) || ''}</AmountDescription>\n`;
  xml += '        </Moa>\n';
  xml += '      </AmountDetails>\n';
  xml += '      <AmountDetails>\n';
  xml += '        <Moa amountTypeCode="I-181" currencyCodeList="ISO_4217">\n';
  xml += `          <Amount currencyIdentifier="TND">${Number(invoice.total_tax).toFixed(3)}</Amount>\n`;
  xml += '        </Moa>\n';
  xml += '      </AmountDetails>\n';
  xml += '    </InvoiceMoa>\n';
  ...
  xml += `            <Amount currencyIdentifier="TND">${Number(invoice.stamp_duty || 0).toFixed(3)}</Amount>\n`;
  xml += '          </Moa>\n';
  xml += '        </AmountDetails>\n';
  xml += '      </InvoiceTaxDetails>\n';
  xml += '      <InvoiceTaxDetails>\n';
...
/**
 * Get all invoices for a company by company_id (numeric)
 */
router.get("/invoices/company/:companyId", async (req, res) => {
  try {
...
/**
 * Add new invoice with items and generate XML
 */
  ...
  try {
    ...
    try {
      ...
      if (invoiceData) {
        ...
        // Save XML file
        const xmlFilePath = path.join(invoicesDir, `invoice_${invoiceId}.xml`);
 ...
 */
router.get("/invoices/xml/:id", async (req, res) => {
  try {
    const invoiceId = req.params.id;
    const xmlFilePath = path.join(__dirname, `../invoices/invoice_${invoiceId}.xml`);
...
router.delete("/invoices/delete/:id", async (req, res) => {
  try {
    ...
    const xmlFilePath = path.join(__dirname, `../invoices/invoice_${req.params.id}.xml`);
    if (fs.existsSync(xmlFilePath)) {
